 ...
 * Agent layer.
 *
 * The rules engine is the only source of truth for WHAT the alert level
 * and action are. The agent's only job is to explain that clearly.
 *
 * Uses Groq through its OpenAI compatible chat completions API.
 */
...
- If a userName is provided in the input, open with a brief, natural greeting using their first name only (for example, "Hi John! ...").
- If no userName is provided, skip the greeting entirely.
- Keep responses short: 3 to 5 sentences for a full facility summary, 2 to 3 sentences for a single-zone follow-up question.
...
async function callGroq(messages, { maxTokens = 400 } = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      `Groq API call failed: ${response.status} ${responseText}`
    );
  }
  ...
  if (!data.choices || !data.choices.length) {
    throw new Error(
      `Groq returned no choices: ${JSON.stringify(data)}`
    );
  }
...
${JSON.stringify(facilityResult, null, 2)}`,
  ...
  const groundingAck = {
    role: "assistant",
    content: "Understood. I have the current facility status.",
  };
